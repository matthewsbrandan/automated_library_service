    ...
    <div class="border-bottom py-3 px-3 d-sm-flex align-items-center">
      <div class="btn-group" role="group" aria-label="Basic radio toggle button group">
        <input
          ...
          id="btn-table-book-all" autocomplete="off"
          v-bind:checked="filter === 'all' ? true : false"
          v-on:click="filter = 'all'"
        >
        <label class="btn btn-white px-3 mb-0" for="btn-table-book-all">Todos</label>
        <input
          type="radio" class="btn-check"
          id="btn-table-book-reserved" autocomplete="off"
          v-bind:checked="filter === 'reserved' ? true : false"
          v-on:click="filter = 'reserved'"
        >
        <label class="btn btn-white px-3 mb-0" for="btn-table-book-reserved">Com Reserva</label>
        <input
          type="radio" class="btn-check"
          id="btn-table-book-borrowed" autocomplete="off"
          v-bind:checked="filter === 'borrowed' ? true : false"
          v-on:click="filter = 'borrowed'"
        >
        <label class="btn btn-white px-3 mb-0" for="btn-table-book-borrowed">Emprestados</label>
      </div>
      ...
      <table class="table align-items-center mb-0">
        ...
        <tbody>
          ...
          @foreach ($books as $book)
            @php
              $stocks = $book->getStockResume();
              $hasReserved = count($stocks) > 1 ? $stocks[1]->amount > 0 : false;
              $hasBorrowed = count($stocks) > 2 ? $stocks[2]->amount > 0 : false;
            @endphp
            <tr v-if="(
              filter === 'all' || 
              (filter === 'reserved' && {{ $hasReserved ? 'true':'false' }}) ||
              (filter === 'borrowed' && {{ $hasBorrowed ? 'true':'false' }})
            ) && (
              !search || 
